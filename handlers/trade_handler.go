...
type productCollectionSetup struct {
...
func parseProductCollectionSetup(raw string) productCollectionSetup {
	...
	if strings.TrimSpace(raw) == "" || json.Unmarshal([]byte(raw), &setup) != nil {
		setup.Methods = []string{"pickup", "meetup"}
		return setup
	}
	return setup
}

func collectionMethodEnabled(setup productCollectionSetup, method string) bool {
	for _, enabled := range setup.Methods {
		if strings.EqualFold(strings.TrimSpace(enabled), method) {
			return true
		}
	}
	return false
...
func validateCollectionDateTimeWindow(meetupDate, meetupTime, windowEndTime string) error {
	...
	if _, err := time.ParseInLocation("15:04", timeValue, location); err != nil {
		return fmt.Errorf("Invalid collection time")
	}
	collectionAt, err := time.ParseInLocation("2006-01-02 15:04", dateValue+" "+timeValue, location)
	if err != nil {
		return fmt.Errorf("Invalid collection date")
	}
...
func resolveMeetupCoordinates(location string, lat, lng *float64) (sql.NullFloat64, sql.NullFloat64, error) {
	if lat != nil && lng != nil && validCoordinate(*lat, *lng) {
		return sql.NullFloat64{Float64: *lat, Valid: true}, sql.NullFloat64{Float64: *lng, Valid: true}, nil
	}
...
func (h *TradeHandler) ensureTradeRuntimeColumns() {
	...
	_, _ = h.db.Exec(`ALTER TABLE trades MODIFY COLUMN status ENUM('pending','accepted','accepted_by_one','accepted_by_both','declined','countered','active','ongoing','awaiting_confirmation','awaiting_other_party','completed','did_not_push_through','under_review','cancelled','cancelled_due_to_conflict','auto_completed','expired','broken','history','archived','pending_multiway','multiway_active') DEFAULT 'pending'`)
	_, _ = h.db.Exec("UPDATE trades SET last_activity_at = COALESCE(last_activity_at, updated_at, created_at) WHERE last_activity_at IS NULL")
	_, _ = h.db.Exec("CREATE INDEX idx_trades_status_last_activity ON trades(status, last_activity_at)")

	h.ensureArchiveRuntimeColumns("trade_like_loops")
	h.ensureArchiveRuntimeColumns("multiway_trades")
	_, _ = h.db.Exec(`ALTER TABLE trade_like_loops MODIFY COLUMN status ENUM('pending','partially_accepted','accepted','confirmed','ongoing','completed','did_not_push_through','history','rejected','cancelled','cancelled_due_to_conflict','broken','expired','archived') DEFAULT 'pending'`)
	_, _ = h.db.Exec(`ALTER TABLE multiway_trades MODIFY COLUMN status ENUM('searching','pending_user3','pending_initiator_upgrade','waiting_acceptance','user3_accepted','user3_declined','active','completed','cancelled','expired','broken','history','archived','fully_declined') DEFAULT 'searching'`)
	_, _ = h.db.Exec("CREATE INDEX idx_trade_like_loops_status_activity ON trade_like_loops(status, last_activity_at)")
	_, _ = h.db.Exec("CREATE INDEX idx_multiway_trades_status_activity ON multiway_trades(status, last_activity_at)")
}

func (h *TradeHandler) ensureArchiveRuntimeColumns(tableName string) {
	columns := []struct {
		name       string
		definition string
	}{
	...
	for _, col := range columns {
		...
		err := h.db.QueryRow(`
			...
			WHERE TABLE_SCHEMA = DATABASE()
			  AND TABLE_NAME = ?
			  AND COLUMN_NAME = ?
		`, tableName, col.name).Scan(&exists)
		if err != nil || exists > 0 {
			continue
		}
		if _, err := h.db.Exec(fmt.Sprintf("ALTER TABLE %s ADD COLUMN %s %s", tableName, col.name, col.definition)); err != nil {
			log.Printf("Warning: failed to add %s.%s: %v", tableName, col.name, err)
		}
	}
	_, _ = h.db.Exec(fmt.Sprintf("UPDATE %s SET last_activity_at = COALESCE(last_activity_at, updated_at, created_at) WHERE last_activity_at IS NULL", tableName))
}
...
func normalizeLocationForCompare(value string) string {
	return strings.Join(strings.Fields(strings.ToLower(strings.TrimSpace(value))), " ")
}

func (h *TradeHandler) getSelectedTargetPickupPoints(productIDs []int) ([]tradePickupPoint, error) {
	...
	rows, err := h.db.Query(fmt.Sprintf(`
		SELECT id,
		       COALESCE(NULLIF(location, ''), NULLIF(pickup_address, ''), '') AS pickup_address,
		       COALESCE(pickup_latitude, latitude) AS pickup_latitude,
		       COALESCE(pickup_longitude, longitude) AS pickup_longitude
		FROM products
		WHERE id IN (%s)
	`, placeholders), args...)
	if err != nil {
	...
	points := make([]tradePickupPoint, 0, len(productIDs))
	for rows.Next() {
		var point tradePickupPoint
		if err := rows.Scan(&point.ProductID, &point.Address, &point.Lat, &point.Lng); err != nil {
			return nil, err
		}
...
func findRequestedPickupPoint(points []tradePickupPoint, requested string, fallback string) (tradePickupPoint, bool) {
	requestedKey := normalizeLocationForCompare(requested)
	fallbackKey := normalizeLocationForCompare(fallback)
	for _, point := range points {
		pointKey := normalizeLocationForCompare(point.Address)
		if requestedKey != "" && pointKey == requestedKey {
			return point, true
		}
	}
	for _, point := range points {
		pointKey := normalizeLocationForCompare(point.Address)
		if fallbackKey != "" && pointKey == fallbackKey {
			return point, true
		}
	}
	return tradePickupPoint{}, false
}
...
func (h *TradeHandler) CreateTrade(c *fiber.Ctx) error {
	...
	if meetingType != "pickup" && meetingType != "meetup" {
	...
	targetPickupPoints, err := h.getSelectedTargetPickupPoints(append([]int{payload.TargetProductID}, payload.AdditionalTargetProductIDs...))
	if err != nil {
		log.Printf("Error loading selected target pickup locations: %v", err)
		return c.Status(500).JSON(models.APIResponse{Success: false, Error: "Failed to validate pickup locations"})
	}
	if meetingType == "pickup" {
		if len(payload.AdditionalTargetProductIDs) > 0 && strings.TrimSpace(payload.MeetupLocation) != "" {
			point, ok := findRequestedPickupPoint(targetPickupPoints, payload.MeetupLocation, targetPickupAddress)
			if !ok || strings.TrimSpace(point.Address) == "" {
				return c.Status(400).JSON(models.APIResponse{Success: false, Error: "These items have different pickup locations. Please choose one location for this trade."})
			}
			meetupLocation = strings.TrimSpace(point.Address)
			targetPickupLat = point.Lat
			targetPickupLng = point.Lng
			targetLat = sql.NullFloat64{}
			targetLng = sql.NullFloat64{}
		} else {
	...
	if meetupLocation == "" {
		return c.Status(400).JSON(models.APIResponse{Success: false, Error: "Collection location is required"})
	}
	if meetupDate == "" || meetupTime == "" {
	...
	res, err := tx.Exec(`
		INSERT INTO trades 
		(buyer_id, seller_id, target_product_id, status, buyer_accepted, trade_option, meeting_type, delivery_address, delivery_type, delivery_instructions, message, offered_cash_amount, payment_method, buyer_meetup_confirmed, buyer_meetup_location, buyer_meetup_time, meetup_location, meetup_label, meetup_time, meetup_lat, meetup_lng)
		VALUES (?, ?, ?, 'pending', TRUE, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		userID, sellerID, payload.TargetProductID, payload.TradeOption, payload.MeetingType, payload.DeliveryAddress, payload.DeliveryType, payload.DeliveryInstructions, payload.Message, payload.OfferedCashAmount, payload.PaymentMethod, meetupLocation != "" && meetupDateTime != "", sql.NullString{String: meetupLocation, Valid: meetupLocation != ""}, sql.NullString{String: meetupDateTime, Valid: meetupDateTime != ""}, sql.NullString{String: meetupLocation, Valid: meetupLocation != ""}, sql.NullString{String: meetupLocation, Valid: meetupLocation != ""}, sql.NullString{String: meetupDateTime, Valid: meetupDateTime != ""}, meetupLat, meetupLng)
...
func (h *TradeHandler) GetTrades(c *fiber.Ctx) error {
	...
	query := `
        SELECT
		  ...
		  t.last_activity_at, t.archived_at, COALESCE(t.archived_reason, '') as archived_reason,
...
func (h *TradeHandler) UpdateTrade(c *fiber.Ctx) error {
	...
	defer services.TouchTradeActivity(h.db, tradeID)
	...
	case "counter":
		...
		counterMeetupTime := strings.TrimSpace(payload.MeetupTime)
		if strings.TrimSpace(payload.MeetupDate) != "" && counterMeetupTime != "" {
			counterMeetupTime = strings.TrimSpace(payload.MeetupDate) + " " + counterMeetupTime
		}
		if strings.TrimSpace(payload.MeetupLocation) != "" || counterMeetupTime != "" {
			...
			if _, err := tx.Exec(
				"UPDATE trades SET "+locationColumn+"=COALESCE(NULLIF(?, ''), "+locationColumn+"), "+timeColumn+"=COALESCE(NULLIF(?, ''), "+timeColumn+"), buyer_accepted=FALSE, seller_accepted=FALSE WHERE id = ?",
				strings.TrimSpace(payload.MeetupLocation), counterMeetupTime, tradeID,
			); err != nil {
				_ = tx.Rollback()
				return c.Status(500).JSON(models.APIResponse{Success: false, Error: "Failed to update counter logistics"})
			}
		}
	...
	case "confirm_meetup":
		...
		switch userID {
		case buyerID:
			updateQuery = "UPDATE trades SET buyer_meetup_location=?, buyer_meetup_time=?, buyer_meetup_confirmed=TRUE, meetup_label=?, meetup_lat=?, meetup_lng=?, updated_at=CURRENT_TIMESTAMP WHERE id = ?"
		case sellerID:
			updateQuery = "UPDATE trades SET seller_meetup_location=?, seller_meetup_time=?, seller_meetup_confirmed=TRUE, meetup_label=?, meetup_lat=?, meetup_lng=?, updated_at=CURRENT_TIMESTAMP WHERE id = ?"
		default:
			return c.Status(403).JSON(models.APIResponse{Success: false, Error: "Not authorized for this trade"})
		}

		// Update the trade with this party's meetup selection
		_, err = h.db.Exec(updateQuery, payload.MeetupLocation, meetupTimeValue, payload.MeetupLocation, meetupLat, meetupLng, tradeID)
		if err != nil {
	...
	case "confirm_meetup_done":
		...
		needsMeetupBackfill := strings.TrimSpace(meetupLocation) == "" || strings.TrimSpace(meetupTime) == ""
		if strings.TrimSpace(meetupLocation) == "" {
		...
		if !meetupLat.Valid || !meetupLng.Valid {
			...
			if coordErr != nil {
				return c.Status(400).JSON(models.APIResponse{Success: false, Error: "Meetup location is missing coordinates. Please set a mapped meetup point before confirming."})
			}
...
func (h *TradeHandler) GetTrade(c *fiber.Ctx) error {
	...
	query += `,
          ...
          COALESCE(t.meetup_label, '') as meetup_label,
	...
	if tr.MeetupLat == nil || tr.MeetupLng == nil {
		label := strings.TrimSpace(tr.MeetupLabel)
		if label == "" {
			label = strings.TrimSpace(tr.MeetupLocation)
		}
		if label != "" {
			if places, placeErr := services.SearchPlaces(label, nil, nil); placeErr == nil && len(places) > 0 {
				lat := places[0].Latitude
				lng := places[0].Longitude
				tr.MeetupLat = &lat
				tr.MeetupLng = &lng
				if tr.MeetupLabel == "" {
					tr.MeetupLabel = places[0].Name
				}
				_, _ = h.db.Exec(
					"UPDATE trades SET meetup_label=COALESCE(NULLIF(meetup_label, ''), ?), meetup_lat=?, meetup_lng=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
					label, lat, lng, tr.ID,
				)
			}
		}
...
func (h *TradeHandler) SendTradeLoopMessage(c *fiber.Ctx) error {
	...
	services.TouchTradeLoopActivity(h.db, loopID)
	id64, _ := res.LastInsertId()
	var createdAt time.Time
...
func (h *TradeHandler) UpdateTradeLoopMeetup(c *fiber.Ctx) error {
	...
	services.TouchLoopOrChainActivity(h.db, loopID)
...
func (h *TradeHandler) UpdateLegHandoff(c *fiber.Ctx) error {
	...
	services.TouchMultiwayChainActivity(h.db, chainID)
