...
func (h *TradeHandler) UpdateTrade(c *fiber.Ctx) error {
	...
	case "accept":
		...
		publishToUser(buyerID, sseEvent{Type: "trade_updated", Data: fiber.Map{
			...
			"partner_name":  (map[int]string{buyerID: sellerName, sellerID: buyerName})[buyerID],
			"is_incoming":   buyerID != userID,
		}})
		publishToUser(sellerID, sseEvent{Type: "trade_updated", Data: fiber.Map{
			"trade_id":      tradeID,
			"status":        "accepted",
			"product_title": productTitle,
			"partner_name":  (map[int]string{buyerID: sellerName, sellerID: buyerName})[sellerID],
			"is_incoming":   sellerID != userID,
		}})
	...
	case "complete":
		...
		if err == nil {
			...
			if bc && sc {
				...
				_, _ = h.db.Exec("INSERT INTO notifications (user_id, type, message, is_read) VALUES (?, 'trade_update', ?, FALSE)", buyerID, "Trade completed: "+productTitle)
				_, _ = h.db.Exec("INSERT INTO notifications (user_id, type, message, is_read) VALUES (?, 'trade_update', ?, FALSE)", sellerID, "Trade completed: "+productTitle)
			} else {
